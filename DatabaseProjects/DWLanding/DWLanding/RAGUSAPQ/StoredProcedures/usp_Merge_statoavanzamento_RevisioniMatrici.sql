...
CREATE PROCEDURE RAGUSAPQ.usp_Merge_statoavanzamento_RevisioniMatrici
AS
BEGIN
SET NOCOUNT ON;

IF OBJECT_ID('RAGUSAPQ.statoavanzamento_RevisioniMatrici', 'U') IS NULL
BEGIN
	EXEC RAGUSAPQ.usp_Create_statoavanzamento_RevisioniMatrici;
	EXEC RAGUSAPQ.usp_AddIndexes_statoavanzamento_RevisioniMatrici;
END;

MERGE INTO RAGUSAPQ.statoavanzamento_RevisioniMatrici AS TGT
USING RAGUSAPQ.statoavanzamento_RevisioniMatriciView (nolock) AS SRC
ON SRC.[N°] = TGT.[N°]
...
OUTPUT
	...
	'RAGUSAPQ.statoavanzamento_RevisioniMatrici' AS full_olap_table_name,
