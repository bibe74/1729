...
CREATE PROCEDURE setup.usp_Load_columns_MetraSegnalazioniEdilizia
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID('setup.columns_MetraSegnalazioniEdilizia', 'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_MetraSegnalazioniEdilizia;
END;
...
MERGE INTO setup.columns_MetraSegnalazioniEdilizia AS TGT
USING (
	SELECT
		...
		column_scale,
		is_nullable
	FROM setup.all_columns_MetraSegnalazioniEdilizia
) AS SRC
...
UPDATE setup.columns_MetraSegnalazioniEdilizia
