...
CREATE PROCEDURE setup.usp_Load_columns_AX2009
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.columns_AX2009', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_AX2009;
END;
...
MERGE INTO setup.columns_AX2009 AS TGT
USING (
	SELECT
...
WHEN NOT MATCHED 
	AND NOT (
        (SRC.full_table_name = N'dbo.PBAPARAMETERS' AND SRC.column_id = 198)
        OR (SRC.full_table_name = N'dbo.SALESPARAMETERS')
	)
THEN INSERT VALUES (
	full_table_name,
	column_id,
