...
CREATE PROCEDURE setup.usp_Load_tables_MES40RAW
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID('setup.tables_MES40RAW', 'U') IS NULL
BEGIN
	EXEC setup.usp_Create_tables_MES40RAW;
END;

MERGE INTO setup.tables_MES40RAW AS TGT
USING (
	SELECT DISTINCT
		schema_name + N'.' + table_name AS full_table_name,
		0 AS is_olap_table,
		N'MES40RAW' AS olap_schema_name,
