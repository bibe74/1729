...
CREATE PROCEDURE setup.usp_Load_columns_MetraDTE
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.columns_MetraDTE', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_MetraDTE;
END;
...
MERGE INTO setup.columns_MetraDTE AS TGT
USING (
	SELECT
		...
		column_scale,
		is_nullable
	FROM setup.all_columns_wwwMetraIt
) AS SRC
