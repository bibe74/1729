...
CREATE PROCEDURE setup.usp_Load_columns_IRDP
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.columns_IRDP', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_IRDP;
END;
...
MERGE INTO setup.columns_IRDP AS TGT
USING (
	SELECT
		...
		column_scale,
		is_nullable
	FROM setup.all_columns_Metra_RaccoltaDatiProduzione
) AS SRC
...
-- In SQL Server 2012 la questione è a posto
----UPDATE setup.columns_IRDP
