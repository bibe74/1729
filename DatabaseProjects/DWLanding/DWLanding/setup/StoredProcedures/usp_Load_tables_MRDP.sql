...
CREATE PROCEDURE setup.usp_Load_tables_MRDP
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.tables_MRDP', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_tables_MRDP;
END;

MERGE INTO setup.tables_MRDP AS TGT
USING (
	SELECT DISTINCT
		schema_name + N'.' + table_name AS full_table_name,
		0 AS is_olap_table,
		N'MRDP' AS olap_schema_name,
