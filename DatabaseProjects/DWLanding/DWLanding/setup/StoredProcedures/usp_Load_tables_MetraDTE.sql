...
CREATE PROCEDURE setup.usp_Load_tables_MetraDTE
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.tables_MetraDTE', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_tables_MetraDTE;
END;

MERGE INTO setup.tables_MetraDTE AS TGT
USING (
	SELECT DISTINCT
		schema_name + N'.' + table_name AS full_table_name,
		0 AS is_olap_table,
		N'MetraDTE' AS olap_schema_name,
