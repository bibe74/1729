...
CREATE PROCEDURE setup.usp_Load_tables_IMETPQ
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.tables_IMETPQ', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_tables_IMETPQ;
END;

MERGE INTO setup.tables_IMETPQ AS TGT
USING (
	SELECT DISTINCT
		schema_name + N'.' + table_name AS full_table_name,
		0 AS is_olap_table,
		N'IMETPQ' AS olap_schema_name,
