...
CREATE PROCEDURE setup.usp_Load_columns_MRDP
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.columns_MRDP', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_MRDP;
END;
...
MERGE INTO setup.columns_MRDP AS TGT
USING (
	SELECT
		...
		column_scale,
		is_nullable
	FROM setup.all_columns_RaccoltaDatiProduzione
) AS SRC
