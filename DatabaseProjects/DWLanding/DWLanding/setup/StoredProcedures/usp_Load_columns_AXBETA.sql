...
CREATE PROCEDURE setup.usp_Load_columns_AXBETA
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.columns_AXBETA', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_columns_AXBETA;
END;
...
MERGE INTO setup.columns_AXBETA AS TGT
USING (
	SELECT
	...
	FROM setup.all_columns_AXBETA C
    --INNER JOIN setup.tables_AXBETA T ON T.full_table_name = C.schema_name + '.' + C.table_name
) AS SRC
