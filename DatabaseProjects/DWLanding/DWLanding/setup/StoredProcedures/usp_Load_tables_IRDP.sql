...
CREATE PROCEDURE setup.usp_Load_tables_IRDP
AS
BEGIN

SET NOCOUNT ON;

IF OBJECT_ID(N'setup.tables_IRDP', N'U') IS NULL
BEGIN
	EXEC setup.usp_Create_tables_IRDP;
END;

MERGE INTO setup.tables_IRDP AS TGT
USING (
	SELECT DISTINCT
		schema_name + N'.' + table_name AS full_table_name,
		0 AS is_olap_table,
		N'IRDP' AS olap_schema_name,
